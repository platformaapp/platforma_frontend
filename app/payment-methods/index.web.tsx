...
/**
 * Веб-версия: обрабатывает return_url YooKassa
 * https://platformaapp.ru/payment-methods/?status=success|pending|failed&tx=<id>
 * Нет отдельного макета — минимальный центрированный статус в каркасе сайта.
 */
export default function PaymentMethodsIndexScreenWeb() {
  ...
  useEffect(() => {
    ...
    if (!effectiveTransactionId) {
      ...
      // Deferred a tick: this route can be the very first page loaded (direct bank
      // redirect), and navigating before the Root Layout finishes mounting throws on web.
      setTimeout(() => router.replace(paymentsRoute as never), 0);
      return;
    }
...
const styles = StyleSheet.create({
  ...
  successTitle: { fontSize: 40, lineHeight: 36, fontFamily: 'Gramatika-Regular', fontWeight: 'normal', color: '#010101', textAlign: 'center', marginBottom: 12 },
  cardText: { fontSize: 16, fontFamily: 'Gramatika-Regular', fontWeight: 'normal', color: '#010101', textAlign: 'center', marginBottom: 8 },
