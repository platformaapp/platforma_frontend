...
/**
 * Веб-версия "Мои записи" — простые вкладки "События" / "Личные встречи"
 * (как в макете), с отдельным списком прошедших (визуально приглушены).
...
export default function MyEventsScreenWeb() {
  ...
  // "Подключиться к встрече" в шапке — ведёт на ближайшую предстоящую личную встречу, если она есть.
    ...
    return (
      <View key={item.id} style={[styles.card, muted && styles.cardMuted]}>
        ...
        <View style={styles.cardBody}>
          ...
          <View style={styles.cardMetaRow}>
            <Text style={styles.cardAuthor}>Личная встреча</Text>
...
const styles = StyleSheet.create({
  ...
  cardMetaRow: { flexDirection: 'row', justifyContent: 'space-between', gap: 8 },
  cardAuthor: { fontSize: 13, fontFamily: 'Inter-Regular', color: '#687076', flexShrink: 1 },
