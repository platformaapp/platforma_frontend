...
export default function EventDetailScreen() {
  ...
  // After YooKassa redirects back with ?payment=callback — poll GET /api/student/payments/callback
  useEffect(() => {
    ...
    const poll = async () => {
      if (stopped) return;
      const paymentId = yookassaPaymentIdRef.current ?? _pendingYookassaPaymentId;

      // No payment ID stored — fall back to event status endpoint
      if (!paymentId) {
      ...
      try {
        ...
        if (Date.now() - startMs < POLL_TIMEOUT_MS) {
          ...
          // Timed out — show payment as pending (user can refresh)
          setIsPollingPayment(false);
          setEvent((prev) => prev ? {
            ...prev,
            isRegistered: true,
            isPaid: false,
            currentUserParticipation: { status: 'pending', paymentStatus: 'pending' },
          } : prev);
        }
      } catch {
        if (!stopped) setIsPollingPayment(false);
      }
    };
  ...
  async function handleGetPay() {
    ...
    setPayError('');
    try {
      const [token, , cards] = await Promise.all([
        getAuthToken(),
        getAuthRole(),
      ...
      // Send payment_method_id only when a card is already linked.
      // Backend handles the no-card case by returning confirmation_url for YooKassa checkout.
      ...
      if (confirmUrl) {
        ...
        // Native: open in-app browser, then check status
        await WebBrowser.openBrowserAsync(confirmUrl);
        const { status: afterStatus } = await fetchPaymentStatus(event.id, yid);
        if (afterStatus === 'paid') {
          _pendingYookassaPaymentId = null;
          yookassaPaymentIdRef.current = null;
          clearPaymentIdFromSession();
  ...
  const isEventOver = eventStartMs !== null && Date.now() > eventStartMs + MEETING_DURATION_MS;

  // Show join button for registered participants/owner only while event hasn't ended.
  // 90-minute window after start matches the backend's own join guard.
  const canJoinEffective = !paymentPending && !isEventOver && (event.isRegistered || isOwnEvent);
...
const styles = StyleSheet.create({
  ...
  errorText: { padding: 24, textAlign: 'center', fontFamily: 'Gramatika-Regular', fontSize: 16, color: '#181818', marginTop: 60 },
  ...
  mainFooterPrice: { fontSize: 14, fontFamily: 'Gramatika-Regular', color: '#1E1E1E' },
  ...
  paymentPendingText: { fontSize: 13, fontFamily: 'Gramatika-Regular', color: '#856404', textAlign: 'center' },
  paymentPendingRetry: { marginTop: 8, borderWidth: 1, borderColor: '#856404', paddingVertical: 6, paddingHorizontal: 16 },
  paymentPendingRetryText: { fontSize: 12, fontFamily: 'Gramatika-Regular', color: '#856404' },
  ...
  otherCardTitleText: { marginBottom: 14, fontSize: 20, lineHeight: 24, fontFamily: 'Gramatika-Regular', color: '#1E1E1E' },
  otherDescription: { fontSize: 14, lineHeight: 24, fontFamily: 'Gramatika-Regular', color: '#1E1E1E' },
  otherFooter: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-end', minHeight: 44 },
  otherFooterTime: { fontSize: 14, fontFamily: 'Gramatika-Regular', color: '#FFFFFF', backgroundColor: '#1E1E1E', paddingHorizontal: 16, paddingVertical: 10 },
  otherPriceContainer: { borderWidth: 1, borderColor: '#1E1E1E', paddingHorizontal: 14, paddingVertical: 8 },
  otherFooterPrice: { fontSize: 14, fontFamily: 'Gramatika-Regular', color: '#1E1E1E' },
  ...
  modalEventText: { fontSize: 14, fontFamily: 'Gramatika-Regular', color: '#1E1E1E' },
  ...
  modalPayButtonText: { fontSize: 16, fontFamily: 'Gramatika-Regular', color: '#FFFFFF' },
